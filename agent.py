...
class Agent:
    ...
    def _local_repair_fallback(self) -> str:
        """當 Gemini 無法使用時的本地修復邏輯"""
        ...
        for step in range(1, max_steps + 1):
            ...
            try:
                rescue_text = llm.call_gemini_rescue(
                    self.smart_summarize_history(msgs), 
                    stuck_reason=f'Repeating {self.state.last_action} {self.state.repeat_count} times.'
                )
            except Exception as e:
                if "429" in str(e) or "RESOURCE_EXHAUSTED" in str(e):
                    logger.warning("⚠️ Gemini 額度用完，使用本地修復邏輯")
                    # === Fallback：讓 Agent 自己嘗試修復 ===
                    rescue_text = self._local_repair_fallback()
                else:
                    raise e
